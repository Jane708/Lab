...
<body style="padding: 20px">
	...
	<form class="pure-form" method="post" action="/javaweb/user/add">
		<fieldset>
			...
			帳號: <input type="text" name="username" placeholder="請輸入 username"
				required />
			<p />
			密碼: <input type="text" name="password" placeholder="請輸入 password"
				required />
			<p />
			電郵: <input type="email" name="email" placeholder="請輸入 email" required />
			<p />
			權限: <select name="role">
			...
			<table class="pure-table pure-table-bordered">
				...
				<c:forEach var="userDto" items="${ userDtos }">
					<tr>
						...
						<td>${ userDto.role }</td>
						<td><a
							href="/javaweb/user/get?username=${ userDto.username }"
							class="button-secondary pure-button">修改</a></td>
						<td><a href="/javaweb/user/delete?userId=${ userDto.userId }"
							class="button-error pure-button">刪除</a></td>
					</tr>
				</c:forEach>
			</table>
		</fieldset>
