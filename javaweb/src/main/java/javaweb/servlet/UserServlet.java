...
/**
 ...
 修改密碼: GET  /user/update/password (得到修改密碼頁面) (需要被攔截)
 修改密碼: POST /user/update/password (修改密碼處理程序) (需要被攔截)
 查詢商品: POST /products (需要被攔截)
...
public class UserServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(pathInfo == null || pathInfo.equals("/*")) { // 查詢全部
			...
			req.getRequestDispatcher("/WEB-INF/view/help.jsp").forward(req, resp);
		} else if(pathInfo.equals("/update/password")) {  // 修改頁面，原本寫 "update/password" 但少了一個斜線，而無法執行，路徑很重要~
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		switch (pathInfo) {
			...
			case "/update/password":
				...
				return; // 把 break 改成 return 可以避免重導到 /javaweb/user 網頁
		}
